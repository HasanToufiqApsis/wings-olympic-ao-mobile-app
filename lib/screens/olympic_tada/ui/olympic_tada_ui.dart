...
//                   double amount = double.tryParse(item.fareInAmount.toString()) ?? 0.0;
//                   double distance = double.tryParse(item.distanceInKm.toString()) ?? 0.0;
//
...
//
//                                   String totalFare = double.parse(item['fareInAmount'].toString()).toStringAsFixed(2);
//                                   String totalDistance = double.parse(item['distanceInKm'].toString()).toStringAsFixed(1);
//
//                                   return Container(
//                                     padding: const EdgeInsets.all(16),
//                                     decoration: BoxDecoration(
...
//                                       Expanded(
//                                         flex: 3,
//                                         child: LangText(fixedTaDa[index].slug??""),
//                                       ),
//
//                                       const SizedBox(width: 12),
//
//                                       Expanded(
//                                         flex: 2,
//                                         child: LangText("${fixedTaDa[index].amount}"),
//                                       ),
//                                     ],
//                                   );
//                                 },
...
//                             final asyncLeaveModel = ref.watch(extraTaDaForOlympicTaDaProvider);
//                             final draftedTaDaAsync = ref.watch(draftedTaDaProvider);
//                             final vehicleTaDas = ref.watch(selectedOlympicTaDaTypeProvider);
//
...
//                                       if (!_dataLoaded) {
//                                         _dataLoaded = true;
//                                         if (draftedTaDa!=null && draftedTaDa.draftedTaDa!=null && draftedTaDa.draftedTaDa!.isNotEmpty) {
//                                           ref.read(selectedOlympicTaDaTypeProvider.notifier).removeTaDa(index: 0);
//                                         }
//                                         _remarksController.text = draftedTaDa?.remarks ?? "";
//                                         for(var val in draftedTaDa?.draftedTaDa ?? <DraftedTaDa>[]) {
//                                           var index = availableTaDaCat.indexWhere((e)=>e.id==val.costType);
//                                           if(index!=-1) {
//                                             final taDa = availableTaDaCat[index];
//                                             ref.read(selectedOlympicTaDaTypeProvider.notifier).addAnother(
//                                                 taDa: SelectedVehicleWithTaDaOlympic(
//                                                   textEditingController: TextEditingController(text: "${val.cost}"),
//                                                   selectedTaDa: taDa,
//                                                 )
//                                             );
//                                           }
...
//                                                       child: CustomSingleDropdown<ExtraTaDaType>(
//                                                         items: availableTaDaCat
//                                                             .map<DropdownMenuItem<ExtraTaDaType>>(
//                                                               (e) => DropdownMenuItem(
//                                                             value: e,
//                                                             child: Text(
//                                                               e.slug ?? "",
...
//                                                         )
//                                                             .toList(),
//                                                         onChanged: (ExtraTaDaType? val) {
//                                                           ref.read(selectedOlympicTaDaTypeProvider.notifier).updateTaDaType(
...
//                                                     flex: 2,
//                                                     child: IgnorePointer(
//                                                       ignoring: !(vehicleTaDas[index].selectedTaDa?.isTextInput??false),
//                                                       child: Consumer(
...
//                                                         // Add logic
//                                                         ref.read(selectedOlympicTaDaTypeProvider.notifier).addNew();
//                                                       } else {
//                                                         // Remove logic
//                                                         ref.read(selectedOlympicTaDaTypeProvider.notifier).removeTaDa(index: index);
//                                                       }
//                                                     },
//                                                     child: Container(
//                                                       width: 48,
//                                                       height: 48,
//                                                       decoration: BoxDecoration(
//                                                         color: index + 1 == vehicleTaDas.length ? primary : Colors.grey[200],
//                                                         borderRadius: BorderRadius.circular(8),
//                                                       ),
//                                                       child: Icon(
//                                                         index + 1 == vehicleTaDas.length ? Icons.add : Icons.close,
//                                                         color: index + 1 == vehicleTaDas.length ? Colors.white : Colors.black54,
//                                                         size: 20,
//                                                       ),
//                                                     ),
//                                                   ),
...
//                                     );
//                                   },
//                                   error: (_,__) => const SizedBox(), loading: () => const SizedBox(),
//                                 );
//                               },
//                               error: (_,__) => const SizedBox(), loading: () => const SizedBox(),
//                             );
...
//                           builder: (context, ref, _) {
//                             final vehicleTaDas = ref.watch(selectedOlympicTaDaTypeProvider);
//                             final transportCategorySum = fareCharts.fold<num>(0, (sum, fare) => sum + fare.fareInAmount);
//                             final extraTaDaSum = vehicleTaDas.fold<num>(0, (sum, item) {
//                               final cost = double.tryParse(item.textEditingController?.text ?? '0') ?? 0;
//                               return sum + cost;
//                             });
//                             final fixedTaDaSum  = fixedTaDa.map((e) => e.amount ?? 0.0).fold<double>(0.0, (sum, item) => sum + item);
//
//                             final overallSum = transportCategorySum + extraTaDaSum + fixedTaDaSum;
//
//                             return Container(
//                               margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
...
//
//                                   // Rows
//                                   _buildSimpleRow("Transport Cost", transportCategorySum, Colors.blueGrey),
//                                   const SizedBox(height: 12),
//                                   _buildSimpleRow("Fixed TA/DA", fixedTaDaSum, Colors.orange[700]!),
//                                   const SizedBox(height: 12),
//                                   // if(fixedTaDa.isNotEmpty)
//                                   _buildSimpleRow("Extra TA/DA", extraTaDaSum, Colors.orange[700]!),
//
//                                   Padding(
...
//                             children: [
//                               LangText("Remarks", style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.grey[700])),
//                               const SizedBox(height: 12),
//                               Container(
//                                 decoration: BoxDecoration(
//                                   color: Colors.grey[50], // Input field slightly darker
...
//                                   // Ensure InputTextFields has border: InputBorder.none inside
//                                 ),
//                               ),
//                             ],
//                           ),
...
//                             onButton1Pressed: () => _controller.draftTaDa(remarks: _remarksController.text),
//                             onButton2Pressed: () => _controller.submitTaDaToServer(fareCharts: fareCharts, remarks: _remarksController.text),
//                           ),
//                         ),
//                       ],
//                     );
...
//             LangText(title, style: const TextStyle(fontSize: 15, color: Colors.black54, fontWeight: FontWeight.w500)),
//           ],
//         ),
//         LangText(
